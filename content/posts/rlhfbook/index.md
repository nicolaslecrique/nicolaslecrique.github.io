...
There is still a practical issues with the above algorithm: In practice, the variance of the estimator of the gradient is too high: the reward for different prompts can be very different.
...
This gives us a second estimator of the gradient where we chose a baseline to minimize its variance. But to compute it, we need the baseline: $V^{\pi_{\theta}}(s)$!

In practice, we train an llm with a specific head, with a usual MSE loss using the output of the reward model as a ground truth. We call this model the __Value network__ (because it computes the value function).

### TRPO

With the above algorithm, we don't know what is a proper step $\alpha$ to update $\theta$. This is important because we cannot just observe the loss as in a usual training, because there is no ground truth as the training data itself is sampled from the current model.

TRPO aims to adress this by giving us a gradient update that is the biggest "safe" step possible.
