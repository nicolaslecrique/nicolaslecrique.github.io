...
* In practice, the variance of the estimator of the gradient is too high. 
* We don't know what is a proper step $\alpha$ to update $\theta$ (we cannot just observe the loss as in a usual training)
...
$\nabla_{\theta} J(\pi_{\theta})==E_{\tau \sim \pi_{\theta}}[A^{\pi_{\theta}}(s_\tau,a_\tau)\nabla_{\theta} \ln (\pi_\theta(s_\tau|a_\tau))]$
